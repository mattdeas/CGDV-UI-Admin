...
export class PartnerListComponent {
  ...
  ngOnInit() {    
    this.Math = Math;
    this.getPartnerList(1, '', '', this.applyFilter, this.applySorting);
  }
  getPartnerList(pageNo: number, filterData: any, sortingData: any, applyFilter: boolean, applySorting: boolean){
    this.loading = true;
    ...
    this.cmsService.getPartnerList(url).subscribe((res: any) => {
        this.loading = false;
        if (res && res.status === 0) {
          return false;
        }
  ...
  edit(data: any) { 
    this.router.navigate(['/pages/cms/home/partner/edit'],{ queryParams: {'id':data.id} , queryParamsHandling:"merge"  }); 
  }

  delete(viz_id ) {
   if (window.confirm('Are you sure you want to delete?')) {
   this.cmsService.deletePartner(viz_id).subscribe((res: any) => {
        if(res.status){
