...
export class JourneyAddComponent {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
        ...
        if(this.id){
          var url = '?id='+this.id;
          this.cmsService.getJourneyList(url)
          .subscribe((res: any) => {
            if(res.status){
              this.isEdit  = true;
              ...
              this.router.navigate(['/pages/cms/about/journey/list']);
            }
          },
      (error) => {        
  ...
  onSubmit() {
    if(this.model.avatar){
      ...
      if(this.isEdit){
        this.cmsService.updateJourney(this.id, this.model)
        .subscribe((res: any) => {
            if(res.status){
        ...
        this.cmsService.addJourney(this.model)
        .subscribe((res: any) => {
            if(res.status){
              this.toasterService.showSuccess(res.message);
              this.router.navigate(['/pages/cms/about/journey/list']);
