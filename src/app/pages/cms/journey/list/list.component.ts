...
export class JourneyListComponent {
  ...
  ngOnInit() {    
    this.Math = Math;
    this.getJourneyList(1, '', '', this.applyFilter, this.applySorting);
  }
  getJourneyList(pageNo: number, filterData: any, sortingData: any, applyFilter: boolean, applySorting: boolean){
    this.loading = true;
    ...
    if (applyFilter === true && filterData) {
      ...
      url +=  (filterData.nav_title ? ('&nav_title=' + filterData.nav_title) : '');
      url +=  (filterData.header ? ('&header=' + filterData.header) : '')
      url +=  (filterData.content ? ('&content=' + filterData.content) : '')
    }
    this.cmsService.getJourneyList(url).subscribe((res: any) => {
        this.loading = false;
        if (res && res.status === 0) {
          return false;
        }
  ...
  edit(data: any) { 
    this.router.navigate(['/pages/cms/about/journey/edit'],{ queryParams: {'id':data.id} , queryParamsHandling:"merge"  }); 
  }

  delete(viz_id ) {
   if (window.confirm('Are you sure you want to delete?')) {
   this.cmsService.deleteJourney(viz_id).subscribe((res: any) => {
        if(res.status){
